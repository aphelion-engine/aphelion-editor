"""Modern dark-themed timeline with transport, scrubber, and playback options."""
...
from gui.theme import TIMELINE_STYLE
from gui.timeline.scrubber import TimelineScrubber
...
class TimelineWidget(QWidget):
    ...
    def __init__(self, project: Project) -> None:
        ...
        self._out_point: int = project.max_frame
    ...
    def _build_ui(self) -> None:
        ...
        self.scrubber.set_in_point(self._in_point)
        self.scrubber.set_out_point(self._out_point)
    ...
    def _sync_from_project(self) -> None:
        self._out_point = max(self._in_point, self.project.max_frame)
        self.scrubber.set_range(self.project.max_frame)
        self.scrubber.set_in_point(self._in_point)
        self.scrubber.set_out_point(self._out_point)
    ...
    def _sync_frame_ui(self, frame_num: int) -> None:
        ...
        self.timecode_label.setText(self._format_timecode(frame_num))

    def _update_meta(self) -> None:
        duration_frames = self.project.max_frame
        self.meta_label.setText(
            f"{self.project.fps:g} fps  ·  {duration_frames} frames  ·  "
            f"In {self._in_point}  Out {self._out_point}"
        )

    def _format_timecode(self, frame_num: int) -> str:
    ...
    def start_playback(self) -> None:
        """Begin advancing frames at the selected speed."""
        interval_ms = max(1, int(1000 / (self.project.fps * self.playback_speed)))
    ...
    def _advance_frame(self) -> None:
        nxt = self.project.current_frame + 1
        end = self._out_point if self._out_point > self._in_point else self.project.max_frame
        if nxt > end:
            if self.is_looping:
                nxt = self._in_point
            else:
    ...
    def set_frame(self, frame_num: int) -> None:
        """Seek project playhead and notify listeners."""
    ...
    def go_to_in(self) -> None:
        self.set_frame(self._in_point)

    def go_to_out(self) -> None:
        self.set_frame(self._out_point)
    ...
    def clear_range(self) -> None:
        self._on_in_changed(0)
        self._on_out_changed(self.project.max_frame)
    ...
    def _on_out_changed(self, frame: int) -> None:
        self._out_point = max(self._in_point, min(frame, self.project.max_frame))
        self.scrubber.set_out_point(self._out_point)
        self._update_meta()
    ...
    def keyPressEvent(self, event: QKeyEvent | None) -> None:
        """Keyboard transport shortcuts when timeline has focus."""
        if event is None:
            return
        key = event.key()
